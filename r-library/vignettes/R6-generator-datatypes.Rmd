...
* R Dataframes are naturally column based structures. Java is generally written to operate iteratively on streams of objects, which are more like rows.
* R lists are complex untyped structures with more similarity to JSON trees than to Java Collections.
* R is 1 based, java is 0 based for indexes.
* R has typed NA values. Java null is untyped.
...
![class-hierarchy](./RClass.png){#id .class width=80% height=50%}
...
b$bounceNumeric(4.6)
# This works and is correctly round tripped but Java thinks the value is a NaN rather than a NA. This is a bug but has limited impact.
